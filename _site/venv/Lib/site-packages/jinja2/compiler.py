...
class CodeGenerator(NodeVisitor):
    ...
    def macro_body(
        ...
        for idx, arg in enumerate(node.args):
            ...
            try:
                ...
                self.writeline(
                    f'{ref} = undefined("parameter {
                        arg.name!r} was not provided",'
                    f" name={arg.name!r})"
                )
            else:
    ...
    def visit_Template(
        ...
        for name, block in self.blocks.items():
            self.writeline(
                f"{self.func('block_' + name)
                   }(context, missing=missing{envenv}):",
                block,
                1,
            )
            self.indent()
    ...
    def visit_Block(self, node: nodes.Block, frame: Frame) -> None:
        ...
        if node.required:
            ...
            self.indent()
            self.writeline(
                f'raise TemplateRuntimeError("Required block {
                    node.name!r} not found")',
                node,
            )
            self.outdent()
    ...
    def visit_FromImport(self, node: nodes.FromImport, frame: Frame) -> None:
        ...
        for name in node.names:
            ...
            self.writeline(
                f"{frame.symbols.ref(alias)} = undefined(f{
                    message!r}, name={name!r})"
            )
            self.outdent()
            if frame.toplevel:
    ...
    def visit_For(self, node: nodes.For, frame: Frame) -> None:
        ...
        if node.recursive:
            self.writeline(
                f"{self.func('loop')
                   }(reciter, loop_render_func, depth=0):", node
            )
            self.indent()
    ...
    def visit_Name(self, node: nodes.Name, frame: Frame) -> None:
        ...
        if node.ctx == "load":
            ...
            if not (
                ...
                self.write(
                    f"(undefined(name={node.name!r}) if {
                        ref} is missing else {ref})"
                )
                return
    ...
    def visit_TemplateData(self, node: nodes.TemplateData, frame: Frame) -> None:
        try:
            ...
            self.write(
                f"(Markup if context.eval_ctx.autoescape else identity)({
                    node.data!r})"
            )
        ...
        with self._filter_test_common(node, frame, True):
            ...
            if node.node is not None:
                ...
                self.write(
                    ...
                    f" if context.eval_ctx.autoescape else concat({
                        frame.buffer}))"
                )
            elif frame.eval_ctx.autoescape:
